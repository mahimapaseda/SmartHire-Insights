...
import { Search, Filter, Users, Layers, Zap, X, Plus } from 'lucide-react';
...
const MacroKnowledgeGraph = () => {
  ...
  const nodeColor = (node) => {
    if (node._dim) return 'rgba(255,255,255,0.08)';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', height: 'calc(100vh - 120px)' }}>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        {selectedNodes.length > 0 && (
          <div style={{
            ...
            <button style={{ background: 'none', color: 'var(--text-tertiary)', padding: '0 2px' }} onClick={() => setSelected([])}>
              <X size={13} />
            </button>
          </div>
        )}
      ...
      <div style={{ flex: 1, display: 'flex', gap: '1rem', minHeight: 0 }}>
          ...
          <ForceGraph2D
            ...
            nodeCanvasObject={(node, ctx, scale) => {
              ...
              if (scale > 1.2) {
                ...
                ctx.fillText(node.label, node.x, node.y + r + fs * 0.9);
              }
            }}
            nodeRelSize={1}
        ...
        {/* Legend */}
        <div className="card" style={{ width: '180px', padding: '1.25rem', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <p style={{ fontSize: '0.65rem', fontWeight: '700', color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Legend</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
            <LegendItem color="#1a5c38" label="Candidates" count={3} />
            <LegendItem color="#22c55e" label="Skills"     count={3} />
            <LegendItem color="#ef4444" label="Companies"  count={2} />
          </div>
          <div className="divider" />
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
            <p style={{ fontSize: '0.7rem', color: 'var(--text-tertiary)' }}>── HAS_SKILL</p>
            <p style={{ fontSize: '0.7rem', color: 'var(--text-tertiary)' }}>── WORKED_AT</p>
          </div>

          <div className="divider" />
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
            <StatLine icon={<Zap size={12} />}    label="Nodes"    value="8" />
            <StatLine icon={<Layers size={12} />} label="Clusters" value="3" />
          </div>
        </div>
      </div>
    </div>
...
const LegendItem = ({ color, label, count }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
    ...
    <div>
      ...
      <p style={{ fontSize: '0.68rem', color: 'var(--text-tertiary)' }}>{count} nodes</p>
    </div>
  </div>
);
...
const StatLine = ({ icon, label, value }) => (
  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '0.75rem' }}>
    <span style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', color: 'var(--text-tertiary)' }}>{icon}{label}</span>
